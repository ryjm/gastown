...
;; Forward declare the mail detail keymap (defined later in file)
(defvar ogent-gastown-mail-detail-mode-map)

;; Declare section class constructors (generated by defclass at runtime)
;; These suppress byte-compiler warnings for conditionally-defined classes
(declare-function ogent-gastown-root-section "ogent-gastown-status")
(declare-function ogent-gastown-stats-section "ogent-gastown-status")
(declare-function ogent-gastown-deacon-section "ogent-gastown-status")
(declare-function ogent-gastown-witness-section "ogent-gastown-status")
(declare-function ogent-gastown-witness-item-section "ogent-gastown-status")
(declare-function ogent-gastown-hook-section "ogent-gastown-status")
(declare-function ogent-gastown-mail-section "ogent-gastown-status")
(declare-function ogent-gastown-mail-item-section "ogent-gastown-status")
(declare-function ogent-gastown-convoy-section "ogent-gastown-status")
(declare-function ogent-gastown-convoy-item-section "ogent-gastown-status")
(declare-function ogent-gastown-rigs-section "ogent-gastown-status")
(declare-function ogent-gastown-rig-item-section "ogent-gastown-status")
(declare-function ogent-gastown-crew-section "ogent-gastown-status")
(declare-function ogent-gastown-crew-item-section "ogent-gastown-status")
(declare-function ogent-gastown-polecat-section "ogent-gastown-status")
(declare-function ogent-gastown-polecat-item-section "ogent-gastown-status")
(declare-function ogent-gastown-workers-section "ogent-gastown-status")

;; Time parsing - use iso8601 library if available, else fallback
(declare-function iso8601-parse "iso8601" (string))
(defun ogent-gastown--parse-time (string)
  "Parse ISO 8601 time STRING, returning time value or nil."
  (condition-case nil
      (if (fboundp 'iso8601-parse)
          (encode-time (iso8601-parse string))
        ;; Fallback for older Emacs
        (let ((re "\\([0-9]\\{4\\}\\)-\\([0-9]\\{2\\}\\)-\\([0-9]\\{2\\}\\)T\\([0-9]\\{2\\}\\):\\([0-9]\\{2\\}\\):\\([0-9]\\{2\\}\\)"))
...
(defun ogent-gastown--run-async (args callback &optional error-callback raw-output no-cache)
...
If NO-CACHE is non-nil, skip cache lookup and storage."
  ;; Check cache first (unless raw output or no-cache)
  (unless (or raw-output no-cache)
    (when-let ((cached (ogent-gastown--cache-get args)))
  ...
  (let* ((default-directory (or ogent-gastown--town-root
         ...
         (proc nil)
         (timer nil)
         (cache-args (unless (or raw-output no-cache) args)))
                 ...
                 (with-current-buffer (process-buffer process)
                   ...
                   (condition-case err
                       (let ((result (if raw-output
                         ...
                         (when cache-args
                           (ogent-gastown--cache-set cache-args result))
...
(defun ogent-gastown--format-time (iso-time)
  ...
  (if (and iso-time (stringp iso-time) (not (string-empty-p iso-time)))
      (if-let ((time (ogent-gastown--parse-time iso-time)))
    ...
    (let ((section (magit-current-section)))
      (cond
       ...
       ((eq (eieio-object-class-name section) 'ogent-gastown-mail-item-section)
        (ogent-gastown--show-mail-detail (slot-value section 'value)))

       ;; Rig item - show rig status
       ((eq (eieio-object-class-name section) 'ogent-gastown-rig-item-section)
        (let ((rig-name (plist-get (slot-value section 'value) :name)))
          (when rig-name
            (async-shell-command
       ...
       ((eq (eieio-object-class-name section) 'ogent-gastown-crew-item-section)
        (let ((crew-name (plist-get (slot-value section 'value) :name)))
          (when crew-name
            (async-shell-command
       ...
       ((eq (eieio-object-class-name section) 'ogent-gastown-polecat-item-section)
        (let ((polecat-name (plist-get (slot-value section 'value) :name)))
          (when polecat-name
            (async-shell-command
       ...
       ((eq (eieio-object-class-name section) 'ogent-gastown-convoy-item-section)
        (let ((convoy-id (plist-get (slot-value section 'value) :id)))
          (when convoy-id
            (async-shell-command
  ...
  (let ((convoy-id (when ogent-gastown--magit-section-available
                     (let ((section (magit-current-section)))
                       (when (eq (eieio-object-class-name section)
                         ...
                         (plist-get (slot-value section 'value) :id))))))
    (if convoy-id
        (async-shell-command
         (format "%s convoy status %s" ogent-gastown-gt-executable convoy-id)
         "*gt convoy*")
  ...
  (let ((rig (when ogent-gastown--magit-section-available
               (let ((section (magit-current-section)))
                 (when (eq (eieio-object-class-name section)
                   ...
                   (plist-get (slot-value section 'value) :rig))))))
    (if rig
        (async-shell-command
  ...
  (let ((crew-name (when ogent-gastown--magit-section-available
                     (let ((section (magit-current-section)))
                       (when (eq (eieio-object-class-name section)
                         ...
                         (plist-get (slot-value section 'value) :name))))))
    (if crew-name
        (async-shell-command
         (format "%s crew status %s" ogent-gastown-gt-executable crew-name)
         "*gt crew*")
...
(defun ogent-gastown--save-position ()
  ...
  (when ogent-gastown--magit-section-available
    (when-let ((section (magit-current-section)))
      ...
      (when (slot-boundp section 'value)
        (setq ogent-gastown--last-section-value (slot-value section 'value))))))
